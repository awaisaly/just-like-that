...
import { contactEnquirySchema } from '../../../lib/contact';
...
export async function POST(request: Request) {
  ...
  if (contentLength > 100_000) {
    return errorResponse(413, 'PAYLOAD_TOO_LARGE', 'The message is too large');
  }
  ...
  if (!parsed.success) {
    return errorResponse(400, 'VALIDATION_ERROR', 'Check your details and try again');
  }

  try {
    await sendContactEmail(parsed.data, reference);
    return successResponse(reference);
  } catch (error) {
